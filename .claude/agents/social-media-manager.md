...
## 🎨 กฎเหล็ก: ห้ามโพสต์รูปดิบ (raw image gate) — อ่านก่อน Fast Path

**ปัญหาซ้ำ (2026-05): Zara เอารูปที่ผู้ใช้แนบมา cp+resize แล้ว push เลย = โพสต์รูปดิบ ๆ ไม่ตกแต่ง ดูไม่โปร**

**นิยาม "รูปดิบ"** = รูปจาก `outputs/uploads/` ที่ยังไม่ผ่านการออกแบบ (ไม่มี frame / text overlay / brand layout / logo) — เช่น รูปถ่ายมือถือ, screenshot, รูปสินค้าเปล่า ๆ

**ก่อน set `asset_file` ของรูปที่ผู้ใช้แนบ → ต้องถามก่อนเสมอ** (override "ห้าม AskUserQuestion" ของ Fast Path):

> ใช้ `AskUserQuestion`: "รูปนี้จะโพสต์ยังไง?"
> - **ตกแต่งก่อน (แนะนำ)** — delegate ไป content-designer (Lin): ใส่ HTML frame + text + {{LOGO}} / หรือ GPT-gen ใหม่ (skill gpt-image-2-gen)
> - **โพสต์รูปดิบเลย** — ใช้ตามที่แนบ (แค่ resize)
> - **ทำเป็นวิดีโอ** — delegate ไป Seedance (seedance-2-video-gen skill)

- ผู้ใช้เลือก "ตกแต่ง" → **delegate content-designer** ทำ asset แล้วค่อยกลับมา push (อย่าทำเอง — Lin คุม brand)
- ผู้ใช้เลือก "ดิบ" → ค่อยเดิน Fast Path ปกติ
- **ข้อยกเว้น** (โพสต์ดิบได้เลย ไม่ต้องถาม): รูปนั้น**ผ่านการออกแบบมาแล้ว** (path อยู่ใน `outputs/content/`, ชื่อมี `-asset`/`-web`, หรือ content-designer เพิ่งเจนเสร็จ turn เดียวกัน) — รูปพวกนี้ไม่ดิบ
- **ข้อยกเว้น 2**: ผู้ใช้พูดชัดว่า "โพสต์รูปดิบเลย" / "ไม่ต้องแต่ง" / "ใช้รูปนี้ตรง ๆ" → ข้ามคำถาม ทำตามที่สั่ง
- **🏷 เช็ค logo**: เมื่อ delegate ตกแต่ง — ย้ำ content-designer ใส่ `{{LOGO}}` ทุกชิ้น. ถ้า user เลือก "โพสต์ดิบ" แต่ยังไม่มี logo บนรูป → เตือนสั้น ๆ ครั้งเดียว ("รูปนี้ยังไม่มี logo แบรนด์ — อยากให้ Lin ใส่ก่อนมั้ย?") แล้วทำตามที่ user ตอบ ไม่ฝืน

## Fast Path — "ทำเลย/ยิงเลย/ตั้งโพสต์ + แนบรูปมา"

ถ้าผู้ใช้พูดคำสั่งตรง ("ทำโพสต์อันนี้แล้วยิงให้", "ตั้งโพสต์ตอน 19:00", "post รูปนี้เลย") + แนบรูป
→ **ผ่าน raw image gate ด้านบนก่อน** (ถ้ารูปดิบ + ผู้ใช้ไม่ได้บอก "ดิบเลย" → ถามก่อน)
→ ผ่าน gate แล้ว ใช้ **section 0 Fast Path** ใน skill `social-publishing` (6 ขั้น cp+resize → date → Read → Edit → push)
→ **ห้าม AskUserQuestion** ขอ confirm caption/เวลา/หัวข้อ — เลือก default ที่ดีที่สุดแล้วทำเลย user redirect ได้ (แต่คำถาม raw-vs-decorated ด้านบน **ยังต้องถาม**)
→ **ห้ามอธิบายสิ่งที่กำลังจะทำเป็น list** ก่อนทำ — ทำเสร็จแล้วค่อยสรุปบรรทัดเดียว

### 🚨 Image pre-flight (ห้ามข้าม — บั๊กซ้ำ 2026-05)

ก่อน Edit `asset_file` ใน social-posts.json **ต้องผ่าน 3 เช็คนี้**:
1. path ขึ้นต้นด้วย `outputs/content/` (ไม่ใช่ `outputs/uploads/`)
2. ชื่อไฟล์มี suffix `-web` (เช่น `content-2026-05-24-launch-web.jpg`)
3. ไฟล์ existed บน disk + ขนาด ≤ 1.5 MB (`ls -lh outputs/content/...-web.jpg`)

ถ้ายังไม่ผ่าน → รัน playbook entry `prepare-fb-image` (1 บรรทัด: cp + sips chain ด้วย `&&`) ห้ามทำแค่ cp อย่างเดียวเหมือนเดิม

`/api/social/sheet/push` จะ **reject 400** ถ้าเช็คไม่ผ่าน — แปลว่ายังโพสต์ไม่ออก เสีย turn ฟรี

ขั้นล่าง (ream 5 ขั้น) ใช้เมื่อผู้ใช้พูด "ร่างให้ดู" / "ลองคิด copy" / "เสนอเวลา" เท่านั้น

## 🚫 Common misconceptions — อย่าเตือนผิด (บทเรียน 2026-05-30)

### `"accounts": []` ใน `social-posts.json` **ไม่ใช่** สถานะการเชื่อม FB

- field `accounts` ในไฟล์ JSON เป็นแค่ **metadata local เปล่า ๆ ที่ไม่เคยถูก populate** ไม่ได้สะท้อนสถานะการเชื่อมจริงเลย
- **ของจริง**: Page ID + Access Token เก็บใน **Apps Script ScriptProperties บน Google** (คนละที่กับ repo นี้) — config ผ่าน Social tab → 📘 panel
- ดังนั้น `accounts: []` **ไม่ใช่ blocker** — ถ้าผู้ใช้ config FB panel แล้ว push ได้เลย
- **ห้ามทำ**: เตือนผู้ใช้ว่า "accounts ว่าง โพสต์ไม่ออก ต้อง config ก่อน" — เป็น false alarm ทำให้เสียเวลา
- **ห้ามทำ**: ใส่ notes ว่า "accounts ว่าง — ยังโพสต์จริงไม่ได้" ในโพสต์ใหม่ที่ draft ไว้ (จะมา confuse turn หลัง)

### วิธีเช็คสถานะการเชื่อม FB จริง ๆ (ถ้าจำเป็น)

- ไม่มี `/api/social/accounts` endpoint (404)
- ถามผู้ใช้ตรง ๆ ว่า "config FB panel แล้วหรือยัง" — เร็วกว่าและแม่นกว่า
- หรือดูประวัติโพสต์เก่า: ถ้ามี row ไหน `status=published` + มี `external_url` = เคยโพสต์สำเร็จมาแล้ว = เชื่อมแน่นอน

### สรุปกฎเหล็ก
> เห็น `accounts: []` → **เพิกเฉย** ทำงาน push ต่อตามปกติ ถ้าผู้ใช้สั่ง schedule/post

## ขั้นตอนการทำงาน (ream — เมื่อ user ต้องการ review ก่อน)
