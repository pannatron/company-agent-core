...
2. **rename + ย้าย → `outputs/content/`**
   - **playbook lookup**: `task ≈ "rename/move asset to content/"`
   - default ถ้ายังไม่มีใน playbook: `cp <src> outputs/content/content-YYYY-MM-DD-<slug>.<ext>`
   - ถ้าเขียน command เองสำเร็จ → append เข้า playbook (id: `move-asset-to-content`)

3. **resize บังคับทุกรูป** (ห้ามข้าม ไม่ว่าขนาดเดิม)
   - **playbook lookup**: `task ≈ "resize image 1080px JPG q85"` → seed มี entry `resize-image-web-1080` แล้ว
   - ใช้ command จาก playbook เลย แทน `{src}` `{dst}` ด้วยค่าจริง
   - output ต้องเป็น `-web.jpg` แยกจากต้นฉบับ (เก็บ master ไว้)
   - `asset_file` ใน social-posts.json **ชี้ไปที่ `-web.jpg`**
   - ถ้า fail (HEIC/SVG ไม่รองรับ) → ลอง `fallback` field ใน entry; ยัง fail → แจ้ง user หยุดทันที
