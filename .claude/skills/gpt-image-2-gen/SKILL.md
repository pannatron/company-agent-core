...
0. **Load the project key first** — `EVOLINK_API_KEY` lives in `.env.local` at the repo root, NOT the shell env. Before any script call, source it: `set -a; . .env.local; set +a`. Every `gpt-image-gen.sh` invocation must be prefixed this way (or run `EVOLINK_API_KEY=$(grep -m1 EVOLINK_API_KEY .env.local | cut -d= -f2) ...`).
