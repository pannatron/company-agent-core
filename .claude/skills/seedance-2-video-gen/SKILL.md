...
0. **Load the project key first** — `EVOLINK_API_KEY` lives in `.env.local` at the repo root, NOT the shell env. Before any `seedance-gen.sh` call, source it: `set -a; . .env.local; set +a` (prefix every invocation).

1. Check if `EVOLINK_API_KEY` is set (run: `set -a; . .env.local 2>/dev/null; set +a; echo $EVOLINK_API_KEY`)
...
3. **Implementation note for AI agents** — the script is a long-running blocking process. To relay progress without waiting for it to finish:
   1. Run the script in **background mode** (e.g., background shell or async exec)
   2. **Poll every 30–40 seconds** to check for new output lines
   3. Each time you get new `STATUS_UPDATE` lines, **send them to the user immediately** — do not buffer until the end
   4. Keep polling until you see `VIDEO_URL=`, `ERROR`, `POLL_TIMEOUT`, or the process exits
4. **While the script is running**, actively relay progress to the user:
...
5. When complete, share the video URL (valid for 24 hours) and generation time.
...
| `POLL_TIMEOUT: task_id=<id> dashboard=<url>` | Polling exceeded 6 min | Tell user: "Your video may already be done — check <dashboard_url> (task: `<id>`)" |
