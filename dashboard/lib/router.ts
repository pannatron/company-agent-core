...
export function routeMessage(text: string): RouteResult {
  ...
  const rules: Array<{ slug: EmployeeSlug; re: RegExp; label: string }> = [
    ...
    // Social distribution verbs / platform routing
    {
      ...
      re: /(publish|schedule|ตั้งเวลา|ลงโพสต์|คิวโพสต์|engagement|reach|impressions|\bsocial\b|linkedin|facebook\b|\bfb\b|instagram|\big\b|twitter|tiktok|reels?|story|ตาราง(โพสต์|เผยแพร่)|โพสต์\s*(ลง|ขึ้น|ใน))/i,
      label: "Social / publish",
    },
    {
      slug: "finance-analyst",
      re: /(เงิน|finance|cashflow|cash\s*flow|บัญชี|accounting|invoice|งบ|budget|รายได้|revenue|กำไร|profit|ขาดทุน|loss|สลิป|slip|ภาษี|tax|vat|งวด|payroll|เงินเดือน|opex|gross\s*margin|runway)/i,
      label: "การเงิน/บัญชี",
    },
    {
      slug: "sales-rep",
